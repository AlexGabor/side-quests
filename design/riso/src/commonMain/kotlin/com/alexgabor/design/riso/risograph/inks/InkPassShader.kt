...
/**
 * Writes one pass's [spec] into [INK_PASS_SKSL]'s uniforms.
 *
 * Every uniform the shader declares is written on every call: skiko rejects a builder with one left
 * unset, and a stale value would be worse than the rejection anyway.
 */
internal fun ShaderUniforms.setInkPass(spec: InkPassSpec) {
    float3("u_paper", spec.paper.transmittance())
    float("u_minTransmittance", MIN_TRANSMITTANCE)
    ...
    float("u_paperFloor", -ln(1f - spec.tolerance.coerceIn(0f, 0.5f)))

    float3("u_ink", spec.ink.transmittance())
    float3("u_row", spec.row[0], spec.row[1], spec.row[2])

    float2("u_origin", spec.origin.x, spec.origin.y)
    float("u_angle", spec.screenAngle)
    float("u_phase", spec.phase)
    float("u_screen", spec.screen.coerceIn(0f, 1f))
    float("u_dotSize", spec.dotSize.coerceAtLeast(1.5f))
    float("u_mottle", spec.mottle.coerceIn(0f, 1f))
    float("u_mottleSize", spec.mottleSize.coerceAtLeast(1f))
 ...
 * skiko's `RuntimeEffect` on the JVM. Only the binding — how uniforms are set and how the result
 * becomes a `RenderEffect` — differs, and that is what [InkPass]'s actuals are.
 */
